...
- **TP.DocumentArchive** [10000] — `.scratch/otorp/Pages/TPArchives.proto → TP.DocumentArchive`. Two content modes share this root; the discriminator is `TP.SettingsArchive.body` (field 1 of the settings archive reached via field 7; default true) — 1/absent = word-processing, 0 = page-layout [fixture-verified: Pages 26.3.1, G1/G2 golden fixtures + corpus 2026-08-29]. BOTH flavors carry `body_storage` (field 4) — layout docs keep a never-rendered body flow (see gotchas #16), so field-4 presence is not a flavor signal. Word-processing documents render `body_storage`; page-layout documents hold page content in `floating_drawables` (3). Document-level page geometry lives here: `page_width` (30), `page_height` (31), margins (32-37), `page_scale` (38), `orientation` (42). `super` (15) chains to the shared TSA/TSK document records.
