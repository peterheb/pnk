...
- **Placeholder visibility** — two mechanisms, both required for fidelity [inferred: fixture-verified 2026-08-30 on crawl decks 2406adf5 (hidden body with stored text) and 0f9df553 (visible title+body)]:
  1. SlideArchive fields 5/6/30 are bookkeeping refs, NOT paint instructions: a title/body/object placeholder paints only when it ALSO appears in `drawables_z_order` (42) / `owned_drawables` (7). Keynote's per-layout checkboxes (Title/Body/…) add/remove the placeholder from the paint lists while the field-5/6/30 ref stays. The slide-number placeholder (20) lives outside the paint lists and is gated by the navigator's `isSlideNumberVisible` instead.
  2. `KN.SlideStylePropertiesArchive { titlePlaceholderVisibility = 4, bodyPlaceholderVisibility = 5, slideNumberPlaceholderVisibility = 6, objectPlaceholderVisibility = 7 }` [proto: KNArchives.proto:479-487], resolved through the slide style's TSS.StyleArchive parent chain — an explicit false hides that role.
