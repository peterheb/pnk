...
| `TSP.DataInfo` bytes (`Data/<file>`) whose ISO BMFF brand is HEIF/HEIC or AVIF | `MediaAsset.format?: "heic" \| "avif"` — sniffed from the bytes, absent when the extension is right; iOS stores HEIC under .jpg names (bd5599: six `FullSizeRender-N.jpg`, `ftypheic`; 24 HEIC files in 4 corpus documents, 1 Pages + 3 Keynote). The viewer falls back to the drawable's `thumbnail` when the browser cannot decode the file (2026-09-06, pages-3b) |
| `TSWP.ParagraphStylePropertiesArchive.border_positions` (45), else `deprecated_borders` (15) | `ParaStyle.borderSides?: ("top"\|"bottom"\|"left"\|"right")[]` — the sides the paragraph `border` is drawn on; absent = all four. Bits 1 top, 2 bottom, 4 all, 8 left, 16 right [inferred: e2e0bff3 and 0c563c6d store 2 and Pages draws a rule below the paragraph only] (2026-09-06, pages-3b) |
| `TP.SettingsArchive` (7): `creation_date` (26), `orig_template` (25), `language` (21), `hyphenation` (9), `document_is_rtl` (18) | `meta.createdAt` (the stored ISO string, with offset), `PagesDocument.template`, `.language` (only when its primary subtag differs from `meta.locale`'s), `.hyphenation` (true only), `.rightToLeft` (true only). All 323 corpus files carry a date and a template name; 10 differ in language, 10 hyphenate, 1 is RTL (77890685). `footnote_format`/`footnote_numbering`/`facing_pages` are default in every corpus file and stay unmodeled (2026-09-06, pages-3b) |
...
| `table_insertion` (21) / `table_deletion` (22) → `TSWP.ChangeArchive { kind, session, date }`; session → `ChangeSessionArchive.author` → `TSK.AnnotationAuthorArchive.name` | tracked changes: the emitted text is the ACCEPTED view — deleted ranges are omitted, insertions kept — and the markup is `PagesDocument.changes[] { kind, paragraphIndex, text, author?, date? }` (2026-09-05, fixture 55d37c2b: 3 insertions, 1 deletion). Body storage only |
...
| `TSD.ImageArchive.instantAlphaPath` (10, `TSP.Path`) | `ImageDrawable.instantAlphaPath?: CurvePath` — the kept region of an Instant Alpha (background-removed) image, in `naturalSize` space; a viewer clips the image to it. Added 2026-09-05; fixture: icecube c3582f31 slide 1 (Keynote's export draws the map cut out and nothing of a crop window that lies outside the path) `[proto for the field; inferred for the coordinate space: the path spans 37-766 x 0-1285 on a 768x1284 image]` |
| `TSWP.EquationInfoArchive` extension fields on `TSD.ImageArchive` (`equation_source_text` 103 / `equation_source_old` 100, `equation_depth` 102, `equation_text_properties` 101) | `ImageDrawable.equation: { source, format, depthPt?, fontSizePt?, fontName?, color?, displayScale? }` — an Insert > Equation image is the app's PDF rendering of `source` (LaTeX, or MathML when the text starts with `<math`; `format` is inferred from the text). `depthPt` is the baseline depth for inline placement. The text/markdown dumpers emit `source` in place of the image. Added 2026-09-05; fixture: atnf Bayesian deck 0ddd627b (55 equations, all LaTeX). `displayScale` (added later the same day): for an INLINE equation the converter multiplies `common.size` and `depthPt` by x-height(fontName)/0.428 (STIXGeneral-Italic's x-height) and records the factor here, because Keynote sets inline math so its x-height matches the run font (HelveticaNeue 1.208, AvenirNext 1.093, Times Italic 1.005; measured in the export of 0ddd627b and 3775cc34); canvas equations stay 1:1 and carry no field `[inferred]` |
...
| `TSD.ImageArchive` variant data beyond primary/original/thumbnail/svg (adjusted, enhanced) | editing derivatives; viewer shows the primary image. The instant-alpha path is NOT dropped since 2026-09-05: see `ImageDrawable.instantAlphaPath` |
