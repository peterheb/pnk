...
| `TSP.DataInfo` bytes (`Data/<file>`) whose ISO BMFF brand is HEIF/HEIC or AVIF | `MediaAsset.format?: "heic" \| "avif"` — sniffed from the bytes, absent when the extension is right; iOS stores HEIC under .jpg names (bd5599: six `FullSizeRender-N.jpg`, `ftypheic`; 7 Pages + 71 Keynote corpus files carry HEIC). The viewer falls back to the drawable's `thumbnail` when the browser cannot decode the file (2026-09-06, pages-3b) |
