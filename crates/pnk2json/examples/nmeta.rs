...
fn main() {
    ...
    for r in loaded.records.values() {
        ...
        let store = m.msg(4);
        for (field, label) in [
            (21u32, "control_cell_spec_table"),
            (19, "commentStorageTable"),
            (16, "multipleChoiceListFormatTable"),
        ] {
            let Some(id) = store.as_ref().and_then(|s| s.reference(field)) else {
                continue;
            };
                ...
                for rf in [4u32, 9, 10, 12] {
                    if let Some(rid) = e.reference(rf) {
                        if let Some(sub) = loaded.msg(rid) {
                            println!(
                                "    -> ref f{rf} = record {rid} (type {:?})",
                                loaded.record(rid).map(|r| r.type_id)
                            );
                            dump(sub, "       ", 3);
                        }
                    }
