...
fn main() {
    ...
    if mode == "summary" {
        for r in loaded.records.values() {
            if r.name.as_deref() != Some("TST.SummaryModelArchive") { continue; }
            println!("=== summary model {}", r.id);
            if let Some(m) = r.msg.as_ref() { dump(m, "  ", 1); }
        }
        return;
    }
    for r in loaded.records.values() {
        if r.name.as_deref() != Some("TSCH.ChartDrawableArchive") { continue; }
        let Some(ca) = r.msg.as_ref().and_then(|m| m.msg(10000)) else { continue };
        let ca = &ca;
        let title = ca.reference(10).and_then(|id| loaded.msg(id)).and_then(|m| m.msg(10000)).and_then(|e| e.string(23));
        println!("=== chart {} type={:?} title={:?}", r.id, ca.varint(1), title);
        for (label, field) in [("value_axis_nonstyle", 14u32), ("value_axis_style", 13), ("chart_nonstyle", 10), ("chart_style", 9)] {
            for id in ca.references(field) {
                let Some(m) = loaded.msg(id) else { continue };
                println!("  {label} {id}:");
                if let Some(e) = m.msg(10000) { dump(&e, "    ", 1); }
            }
        }
    }
}
